...
export default function HistoryScreen() {
  ...
  const renderFilter = (filter: (typeof filters)[0]) => (
    ...
    >
      {filter.icon}
      <Text style={[styles.filterText, selectedFilter === filter.key && styles.filterTextActive]}>{filter.label}</Text>
    </TouchableOpacity>
  )
  ...
  const renderHistoryItem = ({ item }: { item: CallHistoryItem }) => (
    <TouchableOpacity style={styles.historyItem} onPress={() => startNewCall(item)}>
      ...
      <View style={styles.historyDetails}>
        ...
        {item.isGroup && <Text style={styles.participantsList}>{item.participants.join(", ")}</Text>}
        <View style={styles.historyMeta}>
          <Text style={styles.historyDate}>
            {item.date} • {item.time}
          </Text>
          <Text style={styles.historyDuration}>{item.status === "cancelled" ? "Cancelled" : item.duration}</Text>
        </View>
      </View>
    </TouchableOpacity>
  )
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.filtersContainer}>{filters.map(renderFilter)}</View>
